      ...
      module Mixins
        ...
        module BuiltIn
          ...
          class AnyOf < Expectations::Core::CompositeExpectation
            def expect?(value)
              expectations.any? { |expectation| expectation.expect?(value) }
            end
          end
          ...
          class AllOf < Expectations::Core::CompositeExpectation
            def expect?(value)
              expectations.all? { |expectation| expectation.expect?(value) }
            end
          end
          ...
          class HashWithContent < Expectations::Core::SingleValueCallableExpectation
            ...
            def matches_content_expectations?(actual_value)
              expected_value.each_pair.all? do |exp_key, exp_value|
                return false unless actual_value.key?(exp_key) || actual_value.key?(exp_key.to_s)

                value_from_actual = actual_value.fetch(exp_key) { actual_value.fetch(exp_key.to_s) }
                Expectation.build(exp_value).expect?(value_from_actual)
              end
            end

            def initialize(value)
              fail ArgumentError, "a #{EXPECTED_CLASS} is required" unless value.is_a?(::Hash)
