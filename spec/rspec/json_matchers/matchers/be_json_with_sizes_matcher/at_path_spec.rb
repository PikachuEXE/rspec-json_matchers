...
RSpec.describe(*[
  RSpec::JsonMatchers::Matchers::BeJsonWithSizesMatcher,
  "#at_path",
]) do
  ...
  context "when subject is valid JSON object" do
    ...
    describe "the matcher fails the example when path is invalid" do
      it do
        expect(be_json.with_sizes(1).at_path(".").
          matches?(subject)).to eq(false)
      end
      it do
        expect(be_json.with_sizes(1).at_path(".a.").
          matches?(subject)).to eq(false)
      end
      it do
        expect(be_json.with_sizes(1).at_path("a..c").
          matches?(subject)).to eq(false)
      end
  ...
  context "when subject is valid JSON array" do
    ...
    describe "the matcher fails the example when path is invalid" do
      ...
      it do
        expect(be_json.with_sizes(1).at_path(".0.").
          matches?(subject)).to eq(false)
      end
      it do
        expect(be_json.with_sizes(1).at_path("0..0").
          matches?(subject)).to eq(false)
      end
    ...
    describe "when path is invalid due to containing non-digit" do
      specify "the matcher fails the example for `should`" do
        expect(be_json.with_sizes("whatever").at_path("a").
          matches?(subject)).to eq(false)
      end
