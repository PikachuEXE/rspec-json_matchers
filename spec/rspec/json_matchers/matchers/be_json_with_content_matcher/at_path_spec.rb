  ...
  context "when subject is valid JSON object" do
    ...
    describe "#at_path" do
      it "matches matcher with no path" do
        should be_json.with_content(HashWithContent[{ a: HashWithContent[{ b: HashWithContent[{ c: 1 }] }] }])
      end

      it "matches matcher with path 1 level(s) deep" do
        should be_json.with_content(HashWithContent[{ b: HashWithContent[{ c: 1 }] }]).at_path("a")
      end

      it "matches matcher with path 2 level(s) deep" do
        should be_json.with_content(HashWithContent[{ c: 1 }]).at_path("a.b")
      end
    ...
    describe "when there is data is on the path, and using `should_not`" do
      it "the matcher fails the example" do
        expect(be_json.with_content(Anything).at_path("a.b.c").does_not_match?(subject)).to eq(false)
      end
    end

    describe "the matcher fails the example when path is invalid" do
      it do
        expect(be_json.with_content(Anything).at_path(".").matches?(subject)).to eq(false)
      end
      it do
        expect(be_json.with_content(Anything).at_path(".a.").matches?(subject)).to eq(false)
      end
      it do
        expect(be_json.with_content(Anything).at_path("a..c").matches?(subject)).to eq(false)
      end

      it do
        expect(be_json.with_content(Anything).at_path(".").does_not_match?(subject)).to eq(false)
      end
      it do
        expect(be_json.with_content(Anything).at_path(".a.").does_not_match?(subject)).to eq(false)
      end
      it do
        expect(be_json.with_content(Anything).at_path("a..c").does_not_match?(subject)).to eq(false)
      end
    end
    ...
    context "when subject is an object with digit (as String) in keys" do
      ...
      it "matches matcher with no path" do
        should be_json.with_content(HashWithContent[{ "1" => HashWithContent[{ "2" => 1 }] }])
      end

      it "matches matcher with path 1 level(s) deep" do
        should be_json.with_content(HashWithContent[{ "2" => 1 }]).at_path("1")
      end
